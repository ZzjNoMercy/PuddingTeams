...
建议同时下载 `SHA256SUMS.txt` 校验文件完整性。macOS 正式包经过 Developer ID 签名与 Apple notarization。Windows 构建仅在 Release workflow 配置 Authenticode 凭据时签名；未签名安装包会在 Release 说明中明确标注，Windows 可能显示“未知发布者”或 Microsoft Defender SmartScreen 提示，继续安装前请先核对 SHA-256。
